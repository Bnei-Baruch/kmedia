...
andiUA.andiData = function() {
    ...
    if (andiUA.stat) {
        ...
        andiUA.UASwpSvxf("systembasicinfo") && null != a && -1 == a.indexOf(k) ? andiUA.saveStatisticLocalStorage("systembasicinfo", k) : andiUA.UASwpSvxf("systembasicinfo") || andiUA.saveStatisticLocalStorage("systembasicinfo", k)
    }
	})();
};
andiUA.andiData();
andiUA.runStatisMenuUse = function() {
	(function() {
    var k = null;
    andizxc(document).on("mousedown", '#UAMkOFIJ, #UAbczwv [role="tab"] ,#UAbczwv [role="button"],#UAbczwv input', function() {
        "UAMkOFIJ" != this.id && "UAKfLrafQzinput" != this.id || setTimeout(function() {
            if (andiUA.UAQbURpGCa) k = new Date, k = k.getTime();
            else {
                var a = new Date;
                a = a.getTime();
