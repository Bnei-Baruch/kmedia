class FixFiledateInFiles < ActiveRecord::Migration
  def self.up
    ...
    SQL
    execute sql
    execute "DROP TRIGGER IF EXISTS files_fixdate_update;"
    sql = <<-SQL
    CREATE TRIGGER files_fixdate_update BEFORE UPDATE ON files
    FOR EACH ROW
    BEGIN
      SET NEW.filedate = fixdate(NEW.filedate);
    END;
    SQL
    execute sql

    execute "DROP TRIGGER IF EXISTS files_fixdate_insert;"
    sql = <<-SQL
    CREATE TRIGGER files_fixdate_insert BEFORE INSERT ON files
    FOR EACH ROW
    BEGIN
      SET NEW.filedate = fixdate(NEW.filedate);
    END;
  ...
  DROP TRIGGER IF EXISTS files_fixdate_update;
  DROP TRIGGER IF EXISTS files_fixdate_insert;
    SQL
    esecute sql
  end
end
