class BrowserDetector
  ...
  def self.is_smartphone?(request)
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.match(request.user_agent) rescue false
  end

  def self.is_desktop?(request)
    !is_mobile?(request)
  end

  def self.is_smartphone_or_desktop?(request)
    is_desktop?(request) || (is_mobile?(request) && is_smartphone?(request))
  end
end
